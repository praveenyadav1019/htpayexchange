...
router.post("/withdrawals/:id/approve", async (req, res) => {
  ...
  try {
    ...
    if (!tx || tx.status !== "PENDING") {
      throw new Error("Request invalid or already settled.");
    }
    ...
    const updatedWallet = await Wallet.findOneAndUpdate(
      {
        _id: wallet._id,
        inrBalance: { $gte: inrDebit },
        usdtBalance: { $gte: usdtDebit }
      },
    ...
    if (!updatedWallet) {
      throw new Error("Financial inconsistency detected. Settlement aborted.");
    }
  ...
  res.json(
    users.map((u) => {
      const wallet = wallets.find(
        (w) => w.user.toString() === u._id.toString()
      );
      return {
        ...u,
        trustBalance: wallet?.inrBalance || 0,
        incomeBalance: wallet?.usdtBalance || 0
      };
    })
  );
});
